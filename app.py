...
def override_sql_dates_by_question(sql: str, template_key: str, user_question: str) -> str:
    """
    Replace date literals in SQL when user explicitly specifies month/year.
    Works for:
      - SALES_TOTAL_CURR : expects 2 date literals (cur_start, cur_end)
      - *_VS_PREV       : expects 4 date literals (cur_start, cur_end, prev_start, prev_end)
    """
    ...
    prev_start, prev_end = month_range((date(year, month, 1) - relativedelta(months=1)).year,
                                       (date(year, month, 1) - relativedelta(months=1)).month)
    ...
    if template_key == "SALES_TOTAL_CURR":
        # replace first two date literals
        new_sql = re.sub(r"'(\d{4}-\d{2}-\d{2})'", f"'{cur_start}'", new_sql, count=1)
        new_sql = re.sub(r"'(\d{4}-\d{2}-\d{2})'", f"'{cur_end}'", new_sql, count=1)
        return new_sql

    if template_key.endswith("_VS_PREV"):
        # replace first four date literals
        new_sql = re.sub(r"'(\d{4}-\d{2}-\d{2})'", f"'{cur_start}'", new_sql, count=1)
        new_sql = re.sub(r"'(\d{4}-\d{2}-\d{2})'", f"'{cur_end}'", new_sql, count=1)
        new_sql = re.sub(r"'(\d{4}-\d{2}-\d{2})'", f"'{prev_start}'", new_sql, count=1)
        new_sql = re.sub(r"'(\d{4}-\d{2}-\d{2})'", f"'{prev_end}'", new_sql, count=1)
        return new_sql
