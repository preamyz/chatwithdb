...
# --- Domain selector (Step 2: Manual; Step 3: Auto router will be enhanced) ---
domain_mode = st.radio("Domain mode", ["Manual", "Auto"], horizontal=True)

manual_domain = None
if domain_mode == "Manual":
    manual_domain = st.selectbox("Select domain", ["sales", "credit"], index=0)

def guess_domain_rule(q: str) -> str:
    q = (q or "").lower()
    credit_kw = ["เครดิต", "อนุมัติ", "reject", "ปฏิเสธ", "leadtime", "npl", "dpd", "loan", "bureau", "underwriting", "cancellation", "ยกเลิก", "สัญญา"]
    sales_kw  = ["ยอดขาย", "แคมเปญ", "campaign", "สินค้า", "product", "sales", "รุ่น", "model", "dealer", "สาขา"]
    if any(k in q for k in credit_kw):
        return "credit"
...
# Pre-filter QB by domain
auto_domain = None
selected_domain = None
if manual_domain:
    selected_domain = manual_domain
else:
    auto_domain = guess_domain_rule(user_question)
    selected_domain = auto_domain
    st.caption(f"Auto domain guess: **{selected_domain}**")
